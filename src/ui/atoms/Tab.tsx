...
const className = (child, current, variant) => {
  switch (variant) {
    case 1:
      return current === child.key ? style.first.selected : style.first.notSelected;
    case 2:
      return current === child.key ? style.second.selected : style.second.notSelected;
    case 3:
      return current === child.key ? style.third.selected : style.third.notSelected;
    default:
...
export const Tabs = ({ children, variant }: TabsProps) => {
  ...
  const [current, setCurrent] = React.useState<React.ReactChild>(childrenArray[0].key);
